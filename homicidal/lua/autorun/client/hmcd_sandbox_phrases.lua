...
local function HMCD_OpenPhraseMenu(elements)
...
local function HMCD_ClosePhraseMenu()
	PhraseOpen = false
	LocalPlayer():SetNWBool("Phraseopen", false)
    -- Only disable clicker if other menus aren't open (basic check)
    -- In hmcd_sandbox_radial.lua we also manage clicker. 
    -- Ideally we'd have a central manager, but for now:
    if not LocalPlayer():GetNWBool("radialopen") and not LocalPlayer():GetNWBool("Menuuse") then
...
local function getSelected()
	...
	local total = #ments
	if total == 0 then return 0 end -- Fix division by zero if empty
...
local function HMCD_PhraseMousePressed(code, vec)
	if PhraseOpen then
...
concommand.Add(
	"-phrase",
	function(client, com, args, full)
		HMCD_PhraseMousePressed(MOUSE_LEFT)
	end
)
...
local function HMCD_DrawPhraseMenu()
	if PhraseOpen then
		local sw,sh = ScrW(), ScrH()
		local total = #ments
		local w = math.min(sw * 0.45, sh * 0.45)
...
hook.Add("HUDPaint", "HMCD_DrawPhraseMenu", HMCD_DrawPhraseMenu)

hook.Add("GUIMousePressed", "HMCD_PhraseClick", function(code, vec)
	if PhraseOpen and code == MOUSE_LEFT then
		HMCD_PhraseMousePressed(code, vec)
	end
end)
