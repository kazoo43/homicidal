...
concommand.Add("taunt", function (ply, com, args, full)
	local Time=CurTime()
	if !ply.NextTaunt then ply.NextTaunt = 0 end
	if ply.NextTaunt>Time then return end
	if !ply:Alive() then return end
	-- if ply.Otrub then return end -- "Otrub" not defined in sandbox yet?
	...
	if !TauntP[cat][sex] then 
        -- Fallback to male if sex not found (or if ModelSex is nil)
        if TauntP[cat]["male"] then
            sex = "male"
        else
            return 
        end
