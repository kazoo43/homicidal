--[[File Path:   gamemodes/homicide/entities/effects/eff_jack_hmcd_38.lua
...
function EFFECT:Init(data)
	...
	self.Entity:SetColor(Color(255, 215, 0))
...
function EFFECT:GetBulletEjectPos(Position, Ent, Attachment)
	...
	if Ent:IsCarriedByLocalPlayer() and GetViewEntity() == LocalPlayer() then
		...
		if ViewModel:IsValid() then
			...
			if att then return att.Ang, att.Pos + att.Ang:Right() * math.random(1, 3) + att.Ang:Up() * math.random(1, 3) + att.Ang:Forward() * math.random(1, 3) end
		end
