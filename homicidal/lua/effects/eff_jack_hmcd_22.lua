--[[File Path:   gamemodes/homicide/entities/effects/eff_jack_hmcd_22.lua
...
function EFFECT:GetBulletEjectPos(Position, Ent, Attachment)
	...
	if Ent:IsCarriedByLocalPlayer() and GetViewEntity() == LocalPlayer() then
		...
		if ViewModel:IsValid() then
			...
			if att then return att.Ang, att.Pos + att.Ang:Forward() end
		end
		-- Shoot from the world model
	else
		local att = Ent:GetAttachment(Attachment)
		if att then return att.Ang, att.Pos + att.Ang:Forward() * 16 + att.Ang:Up() * 15 + att.Ang:Right() * -28 end
	end
