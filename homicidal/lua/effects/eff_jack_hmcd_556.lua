--[[File Path:   gamemodes/homicide/entities/effects/eff_jack_hmcd_556.lua
...
function EFFECT:Init(data)
	...
	elseif data:GetEntity():GetClass() == "wep_jack_hmcd_assaultrifle" then
		if not (data:GetEntity():IsCarriedByLocalPlayer() and GetViewEntity() == LocalPlayer()) then
			direction = angle:Up()
		end
	elseif data:GetEntity():GetClass() == "wep_jack_hmcd_m249" then
		direction = angle:Forward()
	elseif data:GetEntity():GetClass() == "wep_jack_hmcd_m4a1" then
		direction = angle:Up()
	end
...
function EFFECT:GetBulletEjectPos(Position, Ent, Attachment)
	...
	if Ent:IsCarriedByLocalPlayer() and GetViewEntity() == LocalPlayer() then
		...
		if ViewModel:IsValid() then
			...
			if att and Ent:GetClass() == "wep_jack_hmcd_assaultrifle" then
				return att.Ang, att.Pos + att.Ang:Forward() * -10
			else
		...
		if att then
			if not (Ent:GetClass() == "wep_jack_hmcd_m249") then
				return att.Ang, att.Pos + att.Ang:Right() * 10 + att.Ang:Forward() * -1 + att.Ang:Up() * 5
			else
				return att.Ang, att.Pos + att.Ang:Right() * -5 + att.Ang:Forward() * 10 + att.Ang:Up() * 4
			end
		end
	end
