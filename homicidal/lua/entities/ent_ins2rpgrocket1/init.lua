AddCSLuaFile("cl_init.lua")
AddCSLuaFile("shared.lua")
include("shared.lua")
local phys, vel, owner, pos, DotProduct, traceDir, trace, ED, distance, relation, forceDir, pos2
local td = {}
ENT.JIBFS_NoBlock = true

function ENT:Initialize()
	self:SetModel("models/weapons/tfa_ins2/w_rpg7_projectile.mdl")
	self:PhysicsInit(SOLID_VPHYSICS)
	self:SetMoveType(MOVETYPE_VPHYSICS)
	self:SetSolid(SOLID_VPHYSICS)
	self:SetCollisionGroup(COLLISION_GROUP_NONE)
	self:SetGravity(0.3)
	self:SetDTInt(0, 1)
	self.RocketSpeed = 3000
	phys = self:GetPhysicsObject()

	if phys and phys:IsValid() then
		phys:Wake()
	end

	self.RocketSound = CreateSound(self, "weapons/ins2rpg7/rpg_rocket_loop.wav")
	self.RocketSound:SetSoundLevel(95)
	self.RocketSound2 = CreateSound(self, "rpg/at4rpg_detonate_0" .. math.random(1, 3) .. ".wav")
	self.RocketSound2:SetSoundLevel(95)
	self.RocketSound:Play()

	timer.Simple(6, function()
		if IsValid(self) then
			self:SetDTInt(0, 0) -- oh fuck, we're out of fuel!
			vel = self:GetVelocity()
			self:PhysicsInit(SOLID_VPHYSICS)
			self:SetMoveType(MOVETYPE_VPHYSICS)
			self:SetSolid(SOLID_VPHYSICS)
			self:SetCollisionGroup(COLLISION_GROUP_NPC)
			phys = self:GetPhysicsObject()

			if phys and phys:IsValid() then
				phys:Wake()
				phys:SetMass(5)
				phys:SetVelocity(vel)
			end

			self.RocketSound:FadeOut(0.3)
			owner = self:GetOwner()

			if IsValid(owner) and owner:Alive() then
				owner.CanReload = true
			end
		end
	end)

	spd = physenv.GetPerformanceSettings()
	spd.MaxVelocity = 4528
	physenv.SetPerformanceSettings(spd)
end

function ENT:PhysicsUpdate()
	self:GetPhysicsObject():SetVelocity(self.InitialVel + self.InitialDir * 13500)
end

function ENT:OnTakeDamage(dmginfo)
	return false
end

function ENT:Use(activator, caller)
	return false
end

function ENT:Think()
	if SERVER then
		if self:GetDTInt(0) == 1 then
			owner = self:GetOwner()
			pos = self:GetPos()

			if self.MissileType == 1 then
				self.RocketSpeed = 3839

				if not self.TargetedEntity then
					for k, v in pairs(ents.FindInSphere(pos, 2048)) do
						if (v:IsPlayer() and v:Alive()) or v:IsNPC() then
							pos2 = v:GetPos()
							DotProduct = self:GetForward():DotProduct((pos2 - pos):GetNormal())

							if DotProduct >= 0.7 and v ~= owner then
								traceDir = (pos2 - pos):GetNormal()
								td.start = pos
								td.endpos = td.start + traceDir * 2048 + vector_origin -- make sure we're not running the trace at the entity's feet
								td.filter = {}

								for k2, v2 in pairs(ents.FindByClass("trigger_soundscape")) do
									table.insert(td.filter, v2)
								end

								table.insert(td.filter, self)
								trace = util.TraceLine(td)

								if trace.Entity == v then
									self.TargetedEntity = v
									break
								end
							end
						end
					end
				end

				if not self.TargetedEntity or self.TargetedEntity:GetClass() ~= "ent_ins2rpgrocket" then
					for k, v in pairs(ents.FindInSphere(pos, 1024)) do
						if v:GetClass() == "ent_ins2rpgrocket" and v ~= self then
							pos2 = v:GetPos()
							DotProduct = self:GetForward():DotProduct((pos2 - pos):GetNormal())

							if DotProduct >= 0.6 then
								traceDir = (pos2 - pos):GetNormal()
								td.start = pos
								td.endpos = td.start + traceDir * 1024 + Vector(0, 0, 30) -- make sure we're not running the trace at the entity's feet
								td.filter = {}

								for k2, v2 in pairs(ents.FindByClass("trigger_soundscape")) do
									table.insert(td.filter, v2)
								end

								table.insert(td.filter, self)
								trace = util.TraceLine(td)

								if trace.Entity == v then
									self.TargetedEntity = v
									self.RocketSpeed = 4528
									break
								end
							end
						end
					end
				end

				if IsValid(self.TargetedEntity) then
					if not self.Velocity then
						self.Velocity = self:GetVelocity()
					else
						self.Velocity = LerpVector(0.4, self.Velocity, (self.TargetedEntity:GetPos() - pos):GetNormal() * self.RocketSpeed)
					end

					self:SetLocalVelocity(self.Velocity)
				end
			end

			self:SetAngles(self:GetVelocity():Angle())
		end
	end
end

function ENT:Touch(ent)
	owner = self:GetOwner()

	if ent ~= owner and ent:GetClass() ~= "trigger_soundscape" then
		self.MissileType = 0
		self:Explode()
	end
end

function ENT:PhysicsCollide(data, physobj)
	self.MissileType = 0
	self:Explode()
end

function ENT:OnRemove()
	self.RocketSound:Stop()
	util.ScreenShake(self:GetPos(), 130, 50, 1.75, 900)
end

--util.Effect("sound/mortar_strike_close_02.wav", ED)
function ENT:Explode()
	if self.BlewUp then return end
	owner = self:GetOwner()
	pos = self:GetPos()
	ED = EffectData()
	ED:SetOrigin(pos)
	timer.Simple(.01, function() end) --sound.Play("rpg/at4rpg_detonate_far_dist_0"..math.random(1,3)..".wav",Pos-vector_up,135,100)
	self.RocketSound2:Play()
	ParticleEffect("pcf_jack_groundsplode_small3", pos, Angle(0, 0, 0), nil)

	--ParticleEffect("dusty_explosion_rockets", pos, Angle(0, 0, 0), nil)
	if IsValid(owner) and owner:Alive() then
		owner.CanReload = true
	end

	for key, ent in pairs(ents.FindInSphere(pos, 75)) do
		if (ent ~= self) and (ent:GetClass() == "func_breakable") and ent:CanSee(pos) then
			ent:Fire("break", "", 0)
		elseif (ent ~= self) and HMCD_IsDoor(ent) and not ent:GetNoDraw() and ent:CanSee(pos) and not ent.HasBeenBlasted then
			HMCD_BlastThatDoor(ent)
			ent.HasBeenBlasted = true
		end
	end

	for k, v in pairs(ents.FindInSphere(pos, 256)) do
		if IsValid(v) then
			if not v:IsPlayer() or not v:IsNPC() and (v:GetClass() == "prop_physics" or v:GetClass() == "prop_ragdoll") then
				phys = v:GetPhysicsObject()

				if not v.ReceivedExplosiveForce then
					v.ReceivedExplosiveForce = 300
				else
					v.ReceivedExplosiveForce = v.ReceivedExplosiveForce + 300
				end

				if IsValid(phys) then
					pos2 = v:GetPos()
					distance = pos:Distance(pos2)
					relation = math.Clamp(((256 - distance) / 256) / (phys:GetMass() * 0.005), 0, 1)
					forceDir = ((pos2 - pos):GetNormal() * 5000) * relation

					if phys:IsMoveable() then
						phys:AddAngleVelocity(Vector(1500, 1500, 200) * relation)
						phys:SetVelocity(forceDir)
					end

					if v.ReceivedExplosiveForce >= phys:GetMass() then
						phys:Wake()
						phys:EnableMotion(true)
						phys:AddAngleVelocity(Vector(1500, 1500, 200) * relation)
						phys:SetVelocity(forceDir)
						v.ReceivedExplosiveForce = 0
					end
				end
			end
		end
	end

	if IsValid(owner) then
		util.BlastDamage(self, owner, pos, 350, 225)
	end

	SafeRemoveEntity(self)
end