--[[File Path:   gamemodes/homicide/entities/entities/ent_jack_hmcd_gasparticle.lua
...
function ENT:Think()
	...
	for key, obj in pairs(ents.FindInSphere(SelfPos, 200 * self.Repulsion)) do
		if not (obj == self) and self:Visible(obj) then
			...
			elseif obj:IsPlayer() and obj:Alive() and math.random(5, 15) == 8 then
				HMCD_Poison(obj, self.Owner, "Cyanide")
			end
		end
	end
